...
import { Calendar as CalendarIcon, List, Grid3X3, LayoutGrid, Sparkles, Plus } from "lucide-react";
...
export function CalendarView() {
    ...
    return (
      <div className="w-full max-w-[1600px] mx-auto p-4 space-y-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Calendar Locked</h2>
          <p className="text-gray-600 dark:text-gray-400">Please unlock the app with your master key to view events.</p>
        </div>
      </div>
    );
  }
  ...
  if (isDecrypting) {
    return (
      <div className="w-full max-w-[1600px] mx-auto p-4 space-y-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center">
          <p className="text-gray-600 dark:text-gray-400">Decrypting events...</p>
        </div>
      </div>
    );
  }
  ...
  const generateAIInsights = (analysis: ScheduleWithBuffers) => {
    ...
    if (analysis.optimizationScore >= 80) {
      ...
      insights.push("Consider adding more buffer time between activities to reduce stress.");
    }

    if (analysis.riskAssessment.highRiskEvents.length > 0) {
      insights.push(
        `${analysis.riskAssessment.highRiskEvents.length} events have high delay risk. Add extra time for these.`
      );
    }
    ...
    if (highBufferEvents.length > 0) {
      insights.push(
        `Tip: Add ${highBufferEvents[0].bufferMinutes}min buffer after "${highBufferEvents[0].eventTitle}" to reduce rush.`
      );
    }
  ...
  const handleSaveEvent = async (data: EventFormData) => {
    ...
    try {
      ...
      // Recalculate buffers after save
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      ...
      // Recalculate buffers after delete
      setTimeout(() => loadScheduleAnalysis(), 500);
      setShowModal(false);
      setEditingEvent(null);
      setSelectedSlot(null);
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto p-4 space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 border-b dark:border-gray-700">
          ...
          {scheduleAnalysis && (
            <div className="ml-auto flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">
                Schedule Score: {scheduleAnalysis.optimizationScore}%
              </span>
            </div>
          )}
        </div>
        ...
        {/* AI Insights */}
        {aiInsights.length > 0 && (
          <div className="px-4 py-3 bg-purple-50 dark:bg-purple-900/20 border-b dark:border-gray-700">
